...
**Stage 2 — Lint.** `ruff check` + `ruff format --check` (always). `mypy --strict` on `poc/databricks/lib/` if Python changed. `pytest` + `chispa` with coverage gate ≥ 80%; coverage XML uploaded as an artefact for the PR. Synapse SQL is intentionally not linted — the DDL uses Synapse-specific T-SQL extensions (`DISTRIBUTION = HASH`, `CLUSTERED COLUMNSTORE INDEX`) that the standard `sqlfluff` tsql dialect can't parse; real validation happens at deploy time against a Synapse pool.
