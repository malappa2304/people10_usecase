...
**Stage 2 — Lint.** `ruff check` + `ruff format --check` (always). `mypy --strict` on `poc/databricks/lib/` if Python changed. `pytest` + `chispa` with coverage report uploaded as an artefact (no `--cov-fail-under` gate in PoC scope — only `pipeline_run.py` is comprehensively tested at ~87%; tests for `format_readers.py` and `reconciliation.py` are in TODO.md). Synapse SQL is intentionally not linted — the DDL uses Synapse-specific T-SQL extensions (`DISTRIBUTION = HASH`, `CLUSTERED COLUMNSTORE INDEX`) that the standard `sqlfluff` tsql dialect can't parse; real validation happens at deploy time against a Synapse pool.
